...
class BoxTableCache extends BoxerTableTranslator {
  ...
  BoxTableCache({required String tableName}) : super() {
  ...
  @override
  BoxerTableBase? clone() => BoxTableCache(tableName: this.tableName);
  ...
  Future<Map?> getOneAsMap({int? id, String? type, String? itemId, bool last = false}) async {
    try {
      ...
      BxLoG.d('$tableName getOneAsMap error: $e, $s');
    }
    return null;
  }
  ...
  Future<int> addOne({String? type, String? itemId, dynamic value}) async {
    ...
    values.addAll({
      BoxTableCache.kCOLUMN_ITEM_TYPE: type,
      BoxTableCache.kCOLUMN_ITEM_ID: itemId,
    });
  ...
  BoxerQueryOption getOptions({int? id, String? type, String? itemId}) {
    ...
    List<String> columns = [];
    addIfNotNull(columns, id, BoxTableCache.kCOLUMN_ID);
    addIfNotNull(columns, type, BoxTableCache.kCOLUMN_ITEM_TYPE);
    addIfNotNull(columns, itemId, BoxTableCache.kCOLUMN_ITEM_ID);
    List<String> values = [];
  ...
  Future<int?> maxId() async {
    Object? object = await selectMax(BoxTableCache.kCOLUMN_ID);
    return (object is int) ? object : null;
  }

  Future<int?> minId() async {
    Object? object = await selectMin(BoxTableCache.kCOLUMN_ID);
    return (object is int) ? object : null;
  }
}
